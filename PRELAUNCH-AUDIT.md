...
- 删除无接收能力的假表单；现已接入真实电话、个人微信、邮箱与原始微信二维码。
...
企业主体、备案号、正式域名与未提供的社交主页继续集中标记为“待配置”，未编造替代内容。
...
- 正式域名尚未提供，因此未创建 `CNAME`；真实咨询链路已通过电话、个人微信与邮箱接入。
...
1. 在获得合法资料后，于 `assets/js/site-config.js` 填入企业主体、备案号与社交链接。
2. 联系方式变更时同步更新 `brand-facts.html`、`brand-facts.json` 和 `llms.txt`。
3. 若有正式域名，更新全部canonical、Open Graph URL、JSON-LD、robots、sitemap与配置文件；验证后再添加 `CNAME`。
4. 如需更换个人微信二维码，保留完整矩阵、安全区、真实尺寸、alt与无损PNG压缩。
...
## 联系方式正式接入补充审计（2026-07-23）

- 已接入电话 `18006727611`、个人微信 `gold-king-there` 与邮箱 `1062269723@qq.com`。
- 已将用户提供的个人微信二维码保存为 `assets/images/wechat-qr.png`；原图主体像素保持一致，仅上下各增加5px白色安全区以匹配820×1229规格，并执行无损PNG压缩。
- ZXing验证结果：源二维码文件、桌面联系区域截图、390px手机联系区域截图均识别为QR Code且包含有效内容。
- Playwright验证结果：390、430、1366、1440px通过；电话/邮箱链接正确；复制按钮写入 `gold-king-there` 并显示“微信号已复制”；无控制台错误、页面级横向溢出或可测布局偏移。
- 企业主体、备案号、正式域名与未提供社交主页仍需后续补充。
